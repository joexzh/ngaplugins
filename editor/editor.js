...
loader.css("https://raw.githubusercontent.com/Miruido/ngaplugins/master/editor/editor.css"); //加载CSS
...
function nga_edit_Initialization(){
	nga_plug_addmsg("nga_plug","NGA UBB编辑器插件","1.修改发帖框工具栏高度使之能全部显示。\n2.修复ALT+S快捷键。\n3.修复撤销、重复。\n4.修复快速发帖页面不能使用表情等功能的BUG。");
				...
				for (var k=0;k<nga_plug_mojo[i].data.length;k++){
					if (nga_plug_mojo[i].data[k].check || i == 0){
						...
						for (var l=0;l<nga_plug_mojo[i].data[k].img.length;l++){
							...
							if (i>1) s += '<div onclick="event.cancelBubble = true;nga_edit_mojo(\'click\',this,\'add\');" title="把这个表情添加到自定义表情" style="display:none;position: absolute;background: url(\'https://raw.githubusercontent.com/Miruido/ngaplugins/master/img/add.png\');height:15px;width:15px;border-right:1px solid #777;border-bottom:1px solid #777;"></div>';
							if (i==1) s += '<div onclick="event.cancelBubble = true;nga_edit_mojo(\'click\',this,\'del\');" title="从自定义表情中删除这个表情" style="display:none;position: absolute;background: url(\'https://raw.githubusercontent.com/Miruido/ngaplugins/master/img/del.png\');height:15px;width:15px;border-right:1px solid #777;border-bottom:1px solid #777;"></div>';
						...
						if (i==1){
							...
							s += '<img style="width:40px;height:40px;" src="https://raw.githubusercontent.com/Miruido/ngaplugins/master/img/add1.png"></div>';
						}
